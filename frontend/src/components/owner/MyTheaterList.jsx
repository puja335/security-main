...
export default function MyTheaterList() {
  ...
  useEffect(() => {
    const fetchTheaters = async () => {
      try {
        setLoading(true)
        const response = await axios.get(`${baseUrl}/api/owner/my-theaters`, {
    ...
    <div className='min-h-screen mx-5 my-8 '>
      {theaters.length > 0 ? (
        theaters.map((theater) => (
          <div
            key={theater._id}
            className='bg-base-200 rounded-lg shadow-md p-6 my-4 grid grid-cols-3 gap-4 text-center'
          >
            ...
            <div className='flex flex-col items-end'>
              ...
              <p className='mt-1 text-xl font-semibold'>
                {theater.approved ? (
                  <span className='badge badge-success text-primary-content'>
                    Approved
                  </span>
                ) : (
                  <span className='badge badge-warning text-primary-content'>
                    Pending
                  </span>
                )}
              </p>
