...
export default function MovieStats() {
  ...
  useEffect(() => {
    const fetchTotalMovies = async () => {
      try {
        const res = await axios.get(`${baseUrl}/api/owner/total-movies`, { withCredentials: true });
    ...
    <div className="stats bg-base-200 text-center shadow-lg animate-slide-in-bottom">
      <div className="stat">
        ...
        <div className="stat-actions">
          <Link to='/movie-list' className="btn  btn-sm btn-info text-primary-content">View</Link>
        </div>
      </div>
    </div>
  );
