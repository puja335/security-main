import { addMinutes, format, isAfter, parse, startOfDay } from "date-fns"
  ...
  try {
    ...
    if (existingShows.length > 0) {
      return res.status(400).json({
        message: "Another show exists within 3 hours of the specified time",
      })
    }
...
export const GetShowsByDate = async (req, res) => {
  ...
  try {
    ...
    const shows = await Show.find(query).populate("theater").populate("movieId")
...
export const ShowStats = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ totalShows: shows.length, upComingShows: upComingShows.length })
  } catch (error) {
...
export const getShowByOwner = async (req, res) => {
  ...
  try {
    ...
    const shows = await Show.find({ theater: { $in: theaterIds } }).populate(
      "movieId"
    )

    console.log("the retrieved sows are ", shows)
